...
save_file_name = "cafeInfoLists.xlsx"
write_wb = load_workbook(os.path.join(os.getcwd(), save_file_name))
...
def main():
    ...
    ###################### 서울시 동 주소 불러오기 ############################
    ##########################################################################
    load_url = "C:\\Users\\Guest1\\Desktop\\SSAFY\\cafeSNS\\seoul.xlsx"
    load_wb = load_workbook(load_url, data_only=True)  # 엑셀파일 가져오기
    load_ws = load_wb['Sheet3']  # sheet 이름으로 가져오기
    get_cells = load_ws['A2':'A421']
    ...
    for i, place in enumerate(places):
        ...
        print("#####", i)
        search(place)
        write_wb.save(os.path.join(os.getcwd(), save_file_name))
...
def search(place):
    ...
    for i in range(2, 6):
        ...
        randTime = randint(1, MAX_RAND_INT)
        sleep(randTime)
